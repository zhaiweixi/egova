...
class recInfoFactory(object):
    ...
    def __get_rec_info(self, biz_cur, stat_cur):
        ...
        try:
            wfActInst = (schemaConst.dlhist_ + "to_his_wf_act_inst") if self.__isHisRec else (schemaConst.dlmis_ + "to_wf_act_inst")
            sql = "select act_id from %(wfActInst)s where rec_id = %(rec_id)s and last_act_flag = 1 order by act_id desc"
            param = {"wfActInst": wfActInst, "rec_id": rec_id}
            biz_cur.execute(sql % param)
            if biz_cur.rowcount > 0:
                row = biz_cur.fetchone()
                rec_info["last_act_id"] = row[0]
        except:
        ...
        try:
            if rec_info["act_property_id"] and rec_info["act_property_id"] in (101,):
                wfItemInst = (schemaConst.dlhist_ + "to_his_wf_item_inst") if self.__isHisRec else (schemaConst.dlmis_ + "to_wf_item_inst")
        ...
        # 采集案件共用时长，用archive_time - create_time
        try:
            if rec_info["create_time"] and rec_info["archive_time"]:
                iToatlSeconds = int((rec_info["archive_time"] - rec_info["create_time"]).total_seconds())
                iDays = int(iToatlSeconds/(60*60*24))
                iHours = int((iToatlSeconds % (60*60*24))/(60*60))
                iMinutes = int((iToatlSeconds % (60*60))/60)
                iSeconds = int(iToatlSeconds % 60)
                rec_info["total_cost_time"] = iToatlSeconds
                rec_info["total_cost_time_str"] = str(iDays) + "天" + str(iHours) + "小时" + str(iMinutes) + "分钟" + str(iSeconds) + "秒"
        except Exception, e:
            logger.error("total_cost_time init error[rec_id = %s]: %s" % (rec_id, str(e)))
    ...
    def __get_act_ard_info(self, biz_cur, stat_cur):
        ...
        """
        rec_id = self.__recID
        wfActArd = schemaConst.dlhist_ + "to_his_wf_act_ard" if self.__isHisRec else schemaConst.dlmis_ + "to_wf_act_ard"
        sql = "select act_ard_state_id, apply_act_id, apply_date, apply_part_id, " + \
              "       reply_act_id, reply_date, reply_part_id " + \
              "  from %(wfActArd)s where rec_id = %(rec_id)s"
        param = {"wfActArd": wfActArd, "rec_id": rec_id}
